...
# ╔═╡ 38c0055b-3426-48c3-91f7-ba7d4ea60262
begin
	using CairoMakie
	CairoMakie.activate!(type="svg")
end

# ╔═╡ 9d70439b-9aab-4418-9580-db5f22ec904c
using Colors,LaTeXStrings

# ╔═╡ 9d44b02a-0145-4a06-bf10-dd5690f2d291
using PlutoVista

# ╔═╡ 15898901-620f-415c-9119-b756487b6afa
...
# ╔═╡ 7da1579e-12d2-4f52-8acc-ed8a38d98fa0
md"""
...
# ╔═╡ 29f36323-917f-4b01-94de-c678a2b82e12
md"""
Julia has several plotting packages. In the moment, IMHO there is not the one package
fitting all plotting needs. There are several plotting packages to choose from.
Some of them will be covered in this lecture.
"""

# ╔═╡ 10361866-7bd6-44d7-8fc0-884db1be1113
md"""
## Makie
"""

# ╔═╡ ce2580b2-5574-41e1-b431-7c527b5655de
md"""
Makie is the evolving native Julia plotting library. Due to the design and 
complexity of the code, precompilation times are rather long, howeve this much improved
with Julia 1.9.  It comes
in different incarnations which are useful in different situations.

   - [GLMakie.jl](https://docs.makie.org/stable/explanations/backends/glmakie/index.html) 
   ...
   - [CairoMakie.jl](https://docs.makie.org/stable/explanations/backends/cairomakie/index.html) maps Makie API to the Cairo library, 
         which allows for high quality 2D plots in vector (svg) and bitmap (png) formats. 
     - works well in notebooks
     - uses the browser for rendering when working from the REPL
     - Limited to 2D 
   - [WGLMakie.jl](https://docs.makie.org/stable/explanations/backends/wglmakie/index.html) experimental backend using  three.js for work in the browser
   - [RPRMakie.jl](https://docs.makie.org/stable/explanations/backends/rprmakie/index.html) Experimental ray tracing backend using AMDs RadeonProRender.
The different Makie variants have a relatively long precompilation time, which signficantly improved with Julia 1.9. 
"""



# ╔═╡ e2df071e-5ad4-4e7a-8f40-d75bea271341
md"""
### 1D plots
"""

# ╔═╡ 11a69a0d-95cb-45a9-82b6-c96ca0706e13
md"""
Let us create first plots with CairoMakie (see also [this blog post](https://medium.com/coffee-in-a-klein-bottle/visualizing-data-with-julia-using-makie-7685d7850f06) ):
"""

# ╔═╡ 66ec7308-5f21-43a5-b2c8-ce6cc42fdf9f
X=-10:0.1:10

# ╔═╡ 3cb400c9-a5a0-479b-9743-a48f7a471a35
lines(X,sin.(X))

# ╔═╡ 792a97d9-7392-448a-9bca-f1f5db035298
md"""
...
# ╔═╡ 08bb698f-a8f9-4a50-9fc1-5f7e2a976160
let	
    Φ=-10:0.1:10
    fig=Figure(resolution=(600,300))
    axis=Axis(fig[1,1],xlabel=L"\phi",ylabel="y",title="Example")
    lines!(axis,Φ,sin.(Φ),label=L"sin(\phi)")
    lines!(axis,Φ,cos.(Φ),label=L"cos(\phi)")
    axislegend(axis,bgcolor = RGBA(1.0, 1.0, 1.0, 0.7),position=:lt)
    CairoMakie.save("test.pdf",fig)
    fig
end

# ╔═╡ 725a26ad-e783-4265-aae2-2aa541b932ea
md"""
...
- The `bgcolor` keyword argument makes the axis legend transparent.
...
# ╔═╡ 53b3c2e5-05cc-421a-8166-3d3f039b3b9e
md"""
### 2D plots
"""

# ╔═╡ 449e7592-0aac-4e3a-841f-dac66b50d839
md"""
k: $(@bind k PlutoUI.Slider(0.1:0.05:0.5, default=0.3))
l: $(@bind l PlutoUI.Slider(0.1:0.05:0.5, default=0.2))
"""


# ╔═╡ a9c518ee-047d-472a-b5b6-8060dbc0afca
let	
    X=0:0.05:5
    Y=X
    fig=Figure(resolution=(400,400))
    axis=Axis(fig[1,1],xlabel=L"x",ylabel="y",title="Example")
    F=[sin(k*π*X[i])*sin(l*π*Y[j]) for i=1:length(X), j=1:length(Y)]
    cf=contourf!(axis,X,Y,F,levels=64)	
	CairoMakie.contour!(axis,X,Y,F,levels=5,color=:black,labels=true)
    Colorbar(fig[1,2],cf)
	fig
end


# ╔═╡ 20a9cae2-3ae4-47fe-a751-511186d2dece
md"""
azim: $(@bind azim PlutoUI.Slider(0:0.1π:2π, default=1.75π))
elev: $(@bind elev PlutoUI.Slider(-π/2:0.1π:π/2,default=0.2π))
"""


# ╔═╡ e446ac4d-177e-4d68-a1ad-bac672118c31
let	
    X=0:0.05:5
    Y=X
    fig=Figure(resolution=(400,400))
    axis=Axis3(fig[1,1],xlabel=L"x",ylabel="y",title="Example", 
...
# ╔═╡ 7a68758a-69bb-4bf5-a151-d5faa5bbbf65
md"""
## PlutoVista
"""

# ╔═╡ d1f94813-6cb8-4c61-809d-19d95ce1fc0f
md"""
I created [PlutoVista.jl](https://github.com/j-fu/PlutoVista.jl) for fast plotting in  pluto notebooks. For 1D plots, PlutoVista calls back to Plotly.js, 
and for 2D/3D plots it uses vtk.js, a visualization library for grid and volume data using WebGL as backend.
"""


# ╔═╡ dae3c83e-c05c-4355-93d6-f08407b45047
PlutoVista.plot(X,sin.(2X),xlabel="x",ylabel="y",label="sin",legend=:ct, resolution=(600,300), title="sinus plot")

# ╔═╡ 4b742bd2-bdf5-40eb-bf4d-ab5f696639f3
md"""
k: $(@bind k2 PlutoUI.Slider(0.1:0.05:1, default=0.3))
l: $(@bind l2 PlutoUI.Slider(0.1:0.05:1, default=0.2))
"""

# ╔═╡ 90fa4579-24bc-4c14-b8ad-e9ac3116f8e7
F2=[sin(k2*π*X[i])*sin(l2*π*X[j]) for i=1:length(X), j=1:length(X)];

# ╔═╡ 2da95e51-5a80-4154-a628-adcb74998f8b
PlutoVista.contour(X,X,F2,size=(400,350),levels=5,colormap=:hot)

# ╔═╡ 8f3f9698-039c-489e-9a90-28a7d08db784
md"""
## GridVisualize.jl
"""


# ╔═╡ d071cd82-1f6d-4d38-a9cc-f249e96b45d2
md"""
The [GridVisualize.jl](https://github.com/j-fu/GridVisualize.jl) package focuses on 
PlutoVista (for notebooks) and Makie  as backends.
...
# ╔═╡ 8563387c-6e06-4aeb-a1dc-88a571a6238d
X1=-10:0.5:10

# ╔═╡ 0150de56-921a-443e-8122-6ad150fc0d10
let
	...
	f2=map((x,y)->(sin(x/4)*cos(y/4)),g2)
	scalarplot!(vis[1,2],g2,f2)
    
	g3=simplexgrid(X1,X1,X1)
	f3=map((x,y,z)->(sin(x/3)*cos(y/3)*exp(z/3)),g3)
...
# ╔═╡ 065ef36f-6cb1-4977-9528-762b57ed9fa4
let
	vis=GridVisualizer(;Plotter=CairoMakie,layout=(1,3),size=(700,300))
...
# ╔═╡ d733294d-004b-4f77-8ec2-eac7e29a7ec0
md"""
For 3D graphics, the GLMakie backend is better suited.m
"""

# ╔═╡ 69cc6280-2292-4d78-bbae-e52afcab7e94
md"""
## Plots.jl
"""

# ╔═╡ 9c49d6c3-77a1-43af-82a0-51241b5ec05e
md"""
...
# ╔═╡ 5c51e386-eeb2-45b7-8d3d-9fbc6b0b19ed
md"""
GridVisualize.jl partially supports Plots.jl
"""

# ╔═╡ e8b89db9-7b91-4107-a5f1-467d7f22fadb
md"""
## PyPlot.jl
...
# ╔═╡ e9cda418-1278-4c60-8524-bf26668a7c33
md"""
GridVisualize.jl partially supports PyPlot.jl
"""


# ╔═╡ 389b592f-833d-44e9-8400-1f3b2edbb38b
md"""
## PythonPlot.jl
- [PythonPlot.jl](https://github.com/JuliaPy/PythonPlot.jl): Interface to [python/matplotlib](https://matplotlib.org/)
...
begin
    ...
    highlight(mdstring, color) =
        htl"""<blockquote style="padding: 10px; background-color: $(color); color: #000000 ;">$(mdstring)</blockquote>"""

    macro important_str(s)
        :(highlight(Markdown.parse($s), "#f0aaaa"))
    end
    macro definition_str(s)
        :(highlight(Markdown.parse($s), "#aaaaf0"))
    end
    macro statement_str(s)
        :(highlight(Markdown.parse($s), "#aaf0aa"))
    end
    ...
    html"""
    	...
    	/* Headers */
	/*
         h1{background-color:#dddddd; color: #000000; padding: 10px;}
         h2{background-color:#e7e7e7; color: #000000; padding: 10px;}
         h3{background-color:#eeeeee; color: #000000; padding: 10px;}
         h4{background-color:#f7f7f7; color: #000000; padding: 10px;}
*/
		/* "Terminal"  */
...
manifest_format = "2.0"
project_hash = "d84cad3034df6d142117a77b2c5ea111154540b0"
